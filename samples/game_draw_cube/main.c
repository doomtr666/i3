...
static void init(i3_game_i* game)
{
    ...
    // load the cube model
    i3_content_i* cube_model_content = content_store->load(content_store->self, "cube.bin");

    // create the cube model
    ctx->cube_model = ctx->scene->add_model(ctx->scene->self, cube_model_content);
    // release the model content
    cube_model_content->release(cube_model_content->self);
...
static void update(i3_game_i* game, i3_game_time_t* game_time)
{
    ...
    i3_mat4_t world = i3_mat4_rotation_euler(i3_vec3(2 * game_time->total_time, 3 * game_time->total_time, 0));
    ctx->cube_instance->set_transform(ctx->cube_instance->self, world);
