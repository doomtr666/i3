...
/// A synchronization barrier for an image, buffer, or acceleration structure.
/// AS barriers use global `VkMemoryBarrier2` because AS are accessed via device
/// addresses (no per-resource handle to name in the barrier struct).
#[derive(Debug, Clone)]
pub enum Barrier {
